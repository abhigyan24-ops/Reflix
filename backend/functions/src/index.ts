...
const hmacSecret = defineSecret("hmac-secret");
...
async function publishMQTTCommand(machineId: string, payload: any) {
...
async function sendNotification(uid: string, payload: { title: string; body: string; data?: any }) {
...
async function compileReceiptPDF(data: any): Promise<Buffer> {
  ...
  page.drawText("REFILLX RECEIPT", { x: 50, y: height - 50, size: 18, font: boldFont, color: rgb(0.06, 0.45, 0.74) });
  ...
  page.drawText(`Eco Points: +${data.ecoPointsEarned} points`, { x: 50, y: height - 290, size: 11, font: boldFont, color: rgb(0.06, 0.65, 0.35) });
  ...
  page.drawText("Thank you for contributing to carbon offsets!", { x: 50, y: height - 400, size: 10, font: boldFont, color: rgb(0.06, 0.45, 0.74) });
  
  const bytes = await pdfDoc.save();
  return Buffer.from(bytes);
}

async function compileEcoPassportPDF(data: any): Promise<Buffer> {
  const pdfDoc = await PDFDocument.create();
  ...
  page.drawText("REFILLX ECO PASSPORT", { x: 50, y: height - 60, size: 22, font: boldFont, color: rgb(0.06, 0.65, 0.35) });
  ...
  page.drawText("AGGREGATED IMPACT STATISTICS", { x: 50, y: height - 180, size: 13, font: boldFont, color: rgb(0.06, 0.45, 0.74) });
  ...
  page.drawText(`CO2 Emission Saved: ${data.totalCO2Saved.toFixed(2)} kg`, { x: 50, y: height - 245, size: 11, font: boldFont, color: rgb(0.06, 0.65, 0.35) });
  page.drawText(`PET Plastic Avoided: ${data.totalPlasticSaved.toFixed(0)} grams`, { x: 50, y: height - 265, size: 11, font: boldFont, color: rgb(0.06, 0.65, 0.35) });
  ...
  page.drawText(`Eco Tier Class: ${data.tier}`, { x: 50, y: height - 355, size: 11, font: boldFont, color: rgb(0.85, 0.45, 0.05) });
...
export const onDispenseStart = onMessagePublished(
  "refillx-dispense-start",
  async (event) => {
...
export const onDispenseComplete = onMessagePublished(
  "refillx-dispense-complete",
  async (event) => {
    ...
    try {
      ...
      const transactionResult = await admin.firestore().runTransaction(async (transaction) => {
        ...
        if (!dispenserSnap.exists) throw new Error("Dispenser profile not found.");
        ...
        if (nextPoints >= 1000) nextTier = "Champion";
        else if (nextPoints >= 500) nextTier = "Eco-Hero";
        else if (nextPoints >= 100) nextTier = "Regular";
...
export const onAlertReceived = onMessagePublished(
  "refillx-alerts",
  async (event) => {
    ...
    try {
      ...
      const payload = JSON.parse(payloadString);
      const schema = z.object({
        type: z.enum(["low_stock", "valve_jam", "tamper", "network_dropout"]),
        machineId: z.string(),
        timestamp: z.number(),
      });
...
export const generateEcoPassport = onDocumentCreated(
  "transactions/{txnId}",
  async (event) => {
    ...
    if (!uid || txnData?.type === "credit") return; // Skip wallet deposits
...
export const topUpWallet = onCall(async (request) => {
  ...
  if (!result.success) {
    throw new HttpsError("invalid-argument", "Invalid parameters: " + result.error.message);
  }
  const { uid, amount } = result.data;

  // Enforce deposit bounds (₹10 - ₹10,000)
  if (amount < 10 || amount > 10000) {
    throw new HttpsError("out-of-range", "Wallet deposit amount must be between ₹10 and ₹10,000.");
  }
  ...
  try {
    ...
    if (timestamps.length >= 5) {
      throw new HttpsError("resource-exhausted", "Rate limit exceeded. Maximum 5 top-ups per minute allowed.");
    }
    ...
    throw new HttpsError("internal", error.message || "Failed to complete wallet top-up.");
  }
});

// 3B.7 scheduledAISync (Cloud Scheduler Trigger)
// Runs every 6 hours to fetch predictions from Flask AI Microservice
export const scheduledAISync = onSchedule("every 6 hours", async (event) => {
...
export const generateQRToken = onCall({ secrets: [hmacSecret] }, async (request) => {
  ...
  try {
    ...
    if (timestamps.length >= 10) {
      throw new HttpsError("resource-exhausted", "Rate limit exceeded. Maximum 10 tokens per minute allowed.");
    }
    timestamps.push(nowMs);
    await rateLimitRef.set({ timestamps });
    ...
    throw new HttpsError("internal", error.message || "Failed to generate QR token.");
  }
});
